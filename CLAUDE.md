...
1. **`lineprofiler.LineProfiler`** — a context-manager-based line-by-line profiler. Wrap code in `with profiler:` and it records per-line hit counts and timing for code that lives inside a project folder. Since 0.9.0 it is a *front end*: `line_profiler`'s C callback does the timing (~240 ns/line event) and this package decides what it watches, so nothing is decorated. A pure-Python engine (`engine="builtin"`, ~895 ns) is the fallback where `line_profiler` cannot be imported.
...
- Lint: `poetry run ruff check lineprofiler tests with_line_profiler.py` (config in `pyproject.toml`: line-length 100, target py310, rules `E,F,W,I,N,UP,ANN,B,C4,SIM`, ignoring `ANN101/ANN102` for ruff `^0.5`)
- Type-check: `poetry run mypy lineprofiler tests with_line_profiler.py` (configured `strict = true`, `python_version = 3.10`)
...
- Build: `poetry build` (build backend is `poetry-core`; metadata is PEP 621 `[project]`, the wheel packages the `lineprofiler` dir plus the `with_line_profiler.py` shim)
...
- [tests/test_shim.py](tests/test_shim.py) — packaging rather than profiling: that `import with_line_profiler` re-exports the *same objects* as `lineprofiler`, and that `python -m lineprofiler` prints what `cli.main` prints. Both failures cost a first-time reader minutes and produce an error naming neither cause.
...
The line profiler lives in [lineprofiler/profiler.py](lineprofiler/profiler.py) (~800 lines) and [lineprofiler/engine_lp.py](lineprofiler/engine_lp.py) (~230), with opt-in config in [lineprofiler/config.py](lineprofiler/config.py) and HTML rendering in [lineprofiler/html_source.py](lineprofiler/html_source.py) + the shared [lineprofiler/htmldoc.py](lineprofiler/htmldoc.py). [lineprofiler/__init__.py](lineprofiler/__init__.py) re-exports the public API (`LineProfiler`, `FunctionStats`, `LineStats`, `start_profiling`, `stop_profiling`). Pieces:
...
- `LineProfiler` — the context manager. Core mechanism:
  - **Two engines.** `__enter__` branches on `self._engine`. `line_profiler` delegates to [lineprofiler/engine_lp.py](lineprofiler/engine_lp.py) (below); `builtin` is the pure-Python path described here. `_default_engine()` picks the first where `line_profiler` is importable; passing `backend=` forces `builtin`.
  - **Two builtin backends.** `monitoring` claims `sys.monitoring`'s `PROFILER_ID` slot and registers `_on_line`/`_on_start`/`_on_return`/`_on_throw`/`_on_unwind`; `settrace` registers `self._trace_callback` via **both** `sys.settrace` and `threading.settrace` — the second is what makes worker threads recorded at all. `__exit__` undoes whichever ran. Both share the timing model, `_admits` and `_ensure_function_of`.
  - **`PY_UNWIND` and `PY_THROW` callbacks must not return `DISABLE`.** The interpreter raises `ValueError: Cannot disable PY_UNWIND events` from inside its own dispatch, which surfaces as an unrelated failure far from the callback. They get their own callbacks that return `None`.
  - **The timing model is a per-thread stack of open frames** (`_FrameStack`, a `threading.local`). A frame is pushed on `PY_START`/`call` and popped on `PY_RETURN`/`PY_UNWIND`/`PY_YIELD`/`return`, and the *caller's* clock keeps running across the call — which is what bills a call line for the whole call, the convention every other line profiler uses. One shared "last line" is what this replaced; it billed a call line almost nothing and raced across threads.
  - **`_ensure_function_of` uses `setdefault`.** Two threads reaching a function for the first time at once would otherwise each build a `FunctionStats` and the loser's hits would be billed to an object no longer in the dict.
  ...
  - `_trace_callback` handles `call` / `line` / `return` events, and `_mark_line` / `_close_frame` / `_bill` are the shared timing model both backends drive. The reference timestamp for the next line is taken at the *end* of the callback (a second `perf_counter()` call), so the profiler's own bookkeeping is excluded from the reported per-line times. `_mark_line` opens a frame it has not seen — the `with` block's own frame is already running when profiling starts, so no `PY_START` ever fires for it.
  ...
  - **Regions** (`region()`, `_RegionScope`, `_bill_regions`). `with profiler.region("x"):` partitions the per-line statistics by a named block. Inclusive: a line is billed to *every* region on `_region_stack`, matching the C engine, where every open region's profiler is enabled at once. The stack is per profiler, **not** per thread — a region is a window of the run, and the C engine cannot make it per-thread anyway, since `line_profiler` registers its callbacks on the first enabling thread's manager and every thread then dispatches through it. `_RegionScope` is cached per name and inlines its own bookkeeping rather than calling back into the profiler: at a boundary every Python line executed is a line the profiler is timing, which is why inlining moved the C engine's boundary from ~12 µs to ~7.6 µs.
  - Reporting: `print_stats` (per-function tables), `print_global_top_stats` (top-N lines across all functions), `print_regions` (per-region rankings), `get_stats` (raw dict), and `clear`/`reset` (reset state and caches; `reset` is an alias). Both printers re-check `_is_in_project_folder` before emitting a function, and truncate the source column (50 chars per-function, 40 global) — so output width is fixed at 100 / 130 columns.
...
## Architecture — `lineprofiler/engine_lp.py`

The `line_profiler` engine. `line_profiler` times lines in C but must be *told* which functions to watch; this module does the telling so the caller never lists one. Two invariants carry the whole design, and both trade a rare wrong number for a common missing one:

- **Nothing the caller wrote is modified.** `line_profiler.add_function` disambiguates two functions with identical bytecode by *padding* one, which replaces that function's `__code__`. A function discovered at its own first call is mid-flight when that happens: the running frame keeps the old code, whose line hashes are not the ones just registered, and the whole call goes unrecorded. So functions are registered through a `_CodeHolder` and `_add_without_padding` clears `LineProfiler._all_paddings` around the call (restoring it — it is class-level state shared with any other `line_profiler` user).
- **`_claimed` replaces what padding bought.** `line_profiler` indexes a line by `hash(co_code)` and its line number, so identical bytecode collides wherever line numbers overlap. `_claimed` maps bytecode to the line numbers already registered *in this session* and refuses the second such function, recording it in `skipped`. This is why the engine instance is **per session, not per process**: a process-global one accumulated every function ever profiled, and a script's `busy()` then collided with a test's `late()` from an unrelated earlier session — partial, scrambled counters rather than none.
- **Discovery is `PY_START` on tool slot 4** (2 is `line_profiler`'s own and the builtin engine's, 3 is autotrace), returning `DISABLE` so it costs one callback per code object per session. `restart_events()` on enable, for the same reason the builtin engine needs it. `_register_loaded_modules` and `_register_running_frames` run first, so already-imported functions and the `with` block's own frame are covered; below 3.12 the former is the whole of discovery.
- **`_keys_by_label` translates the keys.** `line_profiler` names a function by `co_qualname`, this package by `co_name`. Without the map a method's rows are dropped, since nothing matches.
- **Two `line_profiler` instances do not agree to the nanosecond.** `inner_trace_callback` reads `hpTimer()` once for billing but stamps each instance's new `last_time` with a *fresh* call inside the per-instance loop, so whichever instance the `set` visits second starts its next interval later. Session and region therefore differ by ~46 ns per line event, in a direction set iteration order decides. It is not a bug to fix here, it must not be normalised away, and `print_regions` says shares are approximate because of it. Anything comparing two instances' totals has to expect either sign.
- **A region is a second `line_profiler.LineProfiler`, enabled only while the region is open** (`_region_lps`, `open_region`/`close_region`). Snapshot-and-difference was measured first and rejected: `get_stats()` costs ~1.5 ms on a 600-function registry, against ~7.6 µs to switch a region on and off. `_register` must add each new function to **every** region profiler as well as the session's, or a region reports a confident zero for code first seen outside it. `_registered` exists so a region created part-way through a session can be given everything registered so far.
...
- `tracealign.py` — pure offline derivation: `usable_anchors` first (a wall clock that steps mid-run makes two of a worker's anchors contradict each other, and fitting a line through the pair maps that bracket onto a reversed, wildly dilated axis — measured: a 40 ms interval placed at −1,440 s and the page headlining `57m 48s` for a 0.25 s run; an anchor is kept only while wall-elapsed and monotonic-elapsed agree within a factor of two, and the rejects are named in `clock_steps` because a silent repair leaves the reader trusting a placement this module does not), then `perf_counter` → common epoch through the anchors, `signal`/`wait_on` matched into arrows, and `critical_path` walking backwards through them. `lane_busy_share` counts a blocked lane as occupied; `lane_working_share` does not — the gap between them is the waiting. `PlacedSpan.depth` is derived here, never recorded: a named phase's path *is* its call stack, so depth is `len(path) - 1`; an auto span's path is a qualname carrying no ancestry, so those are recovered by containment per thread in `_with_derived_depth`.
- `findings.py` — the timeline's *conclusions*: ranked bottlenecks and the phase summary, as pure derivation over an `AlignedTrace`. Two rules produced wrong answers before they were fixed, and both have tests: **a matched `signal`/`wait_on` arrow outranks `concurrent_activity` when calling a wait a queue or a stall** (a producer working in short bursts scores low against a long wait's union, so the learner's `queue_get` — released by an actor every iteration — reported as a stall), and **a parent phase whose wait is spent inside a blocking child is suppressed** (it inherits the child's wait, so reporting both states one fact twice and pushes the real second finding off the list). `_self_times` walks each lane once with a stack; the pairwise containment search it replaced was O(spans²) against a 120k cap. A third rule joined them: **"nothing else was running" is only evidence when there was something else to run** — on a single-lane trace `_explain_wait` says the run recorded one lane and points at the process on the other side of the wait, rather than calling it a stall. `trace_as_dict` lives here too, beside the two derivations it serialises, so `cli.py`'s `trace --format json` and `accounting.write_trace` emit one document instead of two that drift.
...
- `report.py` / `compare.py` / `cli.py` — rendering. The report groups by role and descends past any single-phase level to find where the work actually branches. `_findings_block` prints `rank_findings` above `RESOURCES` and is **silent without a trace** — findings come from spans, and inventing a weaker one from phase totals would put a claim at the top that nothing below it supports. `cli.py`'s `--fail-over` re-merges the run rather than threading findings out of the renderer, so `_render_trace` keeps one return type; it is inert unless a threshold is passed, and still writes the page it failed on. `report` exits **2** for a run directory that does not exist and **0** for one that exists but holds no worker files: the first is a usage error, the second a legitimate answer about a run nobody profiled, which must not fail a pipeline. `accounting/__init__.py` is the library face of the same two commands — `write_report`/`write_trace` share `merge_run` and `trace_as_dict` with `cli.py`, keep their renderer imports lazy inside the function bodies as `cli.py` does, and offer exactly the CLI's formats so there is one story about what a report can be.
...
- **`run_dir=` implies `enabled=True`, but only from the constructor *argument*.** `_resolve_enabled` takes `run_dir is not None` as a flag, never the resolved `self.run_dir`: `_propagate_to_children` exports `LINEPROFILER_RUN_DIR` alongside the switch, so keying on the resolved path would let a `spawn` child enable itself from the inherited variable — including against an explicit `enabled=False` of its own. Resolution order is explicit `enabled`, then explicit `run_dir`, then the environment, then `False`; `enabled=False` wins in both directions. A profiler that ends up disabled counts the phases entered on it (`_disabled_phases`, one int add on the no-op path) and `close()` warns, because the failure it replaces was silent: no file, no directory, exit 0.
...
- `phase(name, sync=True)` calls `torch.cuda.synchronize` at **both** ends. Exit-only would bill the phase for kernels an earlier phase queued. The callable is resolved once into `Profiler._cuda_sync` and bound at scope construction, so a phase that does not synchronise costs one `is not None` test; `torch.cuda.is_available()` walks the driver's device list, which is far too much to ask per phase.
- **`torch.cuda.synchronize()` is what creates a process's CUDA primary context — ~414 MiB of VRAM on an A100 — and `torch.cuda.is_available()` is not.** Measured: `import torch` 0 MiB, `is_available()` returning `True` 0 MiB, the first `synchronize()` 414 MiB. So the resolved callable is *gated*, not eager: `cuda_synchronize()` returns a wrapper that drains only while `torch.cuda.is_initialized()`, because a process with no context has submitted no device work and the drain is a no-op by definition. Without that gate, `sync=True` in a CPU-only actor bought a context it never used, and the cost scaled with the worker count. The check is deliberately **not** cached: a process may initialise CUDA later, and a forked child inherits the callable without the parent's context. `Profiler(cuda_sync=False)` switches it off outright, `True` restores the unconditional drain.
- **A `sync=True` phase's off-CPU time is device compute, not a queue wait.** `wait_ns = wall - cpu` is right for a `queue.get()` and wrong for `.backward()`, where the thread has released the GIL while the *device* works. `FLAG_DEVICE_SYNC` records that a drain actually happened (set in `_record_span` from `Profiler._cuda_live`, so the untraced hot path grows nothing), and `findings._explain_wait` returns a device explanation before it looks at any peer. A finding saying "this is a queue, not a hang" about a backward pass is the wrong-conclusion failure this layer exists to avoid.
- **`findings._releasing_role` only counts arrows that landed *inside* the phase's own spans.** Matching every arrow addressed to the waiting worker let one instrumented queue boundary explain every blocked phase in that process — and it named a producer, which reads as measurement rather than the inference it was. `_SpanIndex` does the containment test by bisect over a running maximum of the end times.
...
- **`cuda_alloc`/`cuda_reserved` and `cuda_proc_used` are two different instruments, and the report shows both.** The first pair is the torch caching allocator, which sees tensors and never sees a CUDA context; `cuda_proc_used` is NVML per pid — the `nvidia-smi` column. Presenting the allocator's peak against a device-total denominator understated a real 2.7 GB as 304.8 MB, and the missing term was exactly the one that scales with worker count. `-1` is the unmeasured sentinel and `0` is a real reading: a process holding no VRAM is the finding, not a gap. Left unmeasured in two cases where zero would be a lie — MIG/vGPU reporting `usedGpuMemory` as `None`, and a pid-namespace mismatch in a container, detected by holding allocator memory while NVML lists no row for us.
...
- **Anything summed across lanes must be divided by lanes, never by `trace.duration_ns`.** Two actors blocked for a whole run wait two lane-seconds per wall second, so a wall-clock denominator produced "costing 155% of the run" at the top of the findings. `Finding.cost_pct` and the phase summary's share both use `duration_ns × lanes`; `_idle_lanes` already did. A share above 100% reads as a bug, not as a sum, and it discredits every correct number under it.
- **A caveat must sit with the number it qualifies, not only in the trailing block.** `CAVEATS` correctly listed three superseded workers while the header said `Processes 1` and the findings drew confident conclusions from the surviving quarter — 80 lines above the disclosure. `_excluded_workers_warning` fires in the header when the excluded outnumber the kept. The usual cause is not a rerun: workers that each build their own `Profiler` without inheriting `LINEPROFILER_RUN_ID` get one attempt id each, and a healthy four-way job reads as four attempts.
- **The phase's own measurement cost lands inside its reported wall time**, ~210–250 ns per entry depending on `measure_cpu`/`trace`, and does not scale with duration. It is within ~25 ns of a bare context manager reading two clocks, so it is not a defect to fix — but it is not subtracted either (that would substitute an estimate for a measurement), and `docs/accounting-recipes.md` states the ratios. `bench_accounting.py`'s second table measures it.
- **The GPU strip is 1 Hz, whole-device, every process, and joins to nothing.** It carries a note saying so, driven off `payload["gpu"]` so it cannot describe a strip the script does not paint. Never present it as if a reading could be attributed to the span above it: that is the async-submission error in reverse, and the page's job is to prevent it.

## Hardening

Reliability work runs as an iterative loop with its own tooling, so that each pass starts from what the last one settled rather than re-deriving it.

- **[HARDENING.md](HARDENING.md)** is the ledger: one row per iteration, recording the surface probed, what was found, the fix and the test. Rows marked *fixed*, *clean* or *not-a-bug* carry their reasoning and are what stop the loop cycling. **Read it before probing anything.**
- **`.claude/skills/harden-loop`** drives one iteration: pick a surface, break it with a real run, judge, fix at the right layer, add the regression test, verify, record. One surface per invocation, taken from red to green.
- **`.claude/skills/adversarial-inputs`** is the input catalogue and the traps that produce phantom bugs (chiefly: `enabled=True` is required or nothing is written at all; never pipe a render through `head`).
- **`.claude/skills/report-readability`** operationalises §5b below — rendering a real page and reading it as a stranger would.

Two invariants established by iteration 1, worth knowing before adding a renderer:

- **Every user-supplied label goes through a bounding chokepoint** — `report.format_label` (which calls `_printable`) for text, `htmldoc.clip_label` plus `htmldoc.escape` for HTML. A new column or page that formats a phase name without one of those reopens both the width and the control-character defect.
- **A measured number is never truncated; a derived one may be compacted.** An overflowing count pushes its row right rather than printing a wrong value (documented in `_counter_rows`); only rates go through `format_rate`. Truncating a label loses nothing recoverable — the full name is in the JSON payload — but truncating a measurement invents one.

## Gotchas

- The `settrace` backend is global and single-tracer, so it cannot coexist with coverage.py or pdb; the `monitoring` backends have their own tool slots and can. All engines are thread-safe as of 0.9.0. Recursive/nested calls share one `FunctionStats` per function key (no per-call-depth breakdown), but their per-line timing is correct because the line is identified from the live `frame` on every event rather than from a single remembered key.
- `print_stats` defaults to `sort_by="line"` (source order) because the table is read against the code; `print_global_top_stats` defaults to time, because ranking is what that table is for. Both take `stream=`.
